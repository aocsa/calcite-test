...
public class PushProjector {
	...
	createProjectRefsAndExprs(RelNode projChild, boolean adjust, boolean rightSide, RelNode origProj) {
		...
		List<RexNode> childExps = logicalProject.getNamedProjects().stream().map(p -> p.left).collect(Collectors.toList());
		for(int i = 0; i < nInputRefs; i++) {
			if(i < childExps.size()) {
				fieldsMap.put(childExps.get(i).hashCode(), fieldNames.get(i));
			}
		}
	...
	for (int i = 0; i < nInputRefs; i++) {
		if (i < total_projected_columns) {
			if (!rightSide) {
				...
				fieldsMap.put(childExps.get(left_total_columns + i).hashCode(), fieldNames.get(left_total_columns + i));
			}
		}
	}
