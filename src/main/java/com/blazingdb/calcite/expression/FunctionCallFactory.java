...
import com.blazingdb.calcite.expression.expr.AnyValueExpression;
import com.blazingdb.calcite.expression.expr.BooleanOperator;
import com.blazingdb.calcite.expression.expr.CastExpression;
import com.blazingdb.calcite.expression.expr.ConvertExpression;
...
public class FunctionCallFactory {
    ...
    public static LogicalExpression createByOp(List<LogicalExpression> args, ExpressionPosition ep, List<String> opTypes) {
        ...
        if (args.size() - 1 != opTypes.size()) {
            throw new RuntimeException("Must receive one more expression then the provided number of operators.");
        }
